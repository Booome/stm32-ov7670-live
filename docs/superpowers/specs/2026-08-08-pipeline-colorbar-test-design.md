...
#### 3.3.5 调试发现：不要复位传感器来"冻结帧"（时钟 bug，2026-08-10）

**现象**：读回 walking-one（tp=01）时，数据只在开头 32B 是有效模式，随后全部变成
`0x33/0xCC` 垃圾；`distinct_bytes` 仅 10。

**根因**：AL422B 内部是 DRAM，需要 WCK/RCK 中较快者持续 free running 作刷新时钟
（datasheet 8.3.1）。测试早期版本在写窗口关闭后调用 `OV7670_RESET_Low()` 复位传感器
以"冻结当前帧"——但复位会**停止 PCLK（=WCK）**，FIFO 立即失去刷新时钟，DRAM 数据毫秒级丢失。

**修复**：删除复位调用。冻结帧无需复位：模块 NAND 门实现 `/WE = NAND(HREF, FIFO_WR)`，
只要 `FIFO_WR` 拉低，无论 HREF/PCLK 如何运行，数据都不会再写入。保持传感器时钟照常
free running 即可保住 FIFO 数据。

**实测对比**（tp=01，`contig dump` 有效 walking-one 长度）：

| 版本 | 有效数据量 | `0x33/0xCC` 计数 | `distinct_bytes` |
|------|-----------|------------------|------------------|
| 复位传感器（有 bug） | 32B | 1985/1984（~97% 垃圾） | 10 |
| 仅 WR 冻结（修复后） | 256B+ 全程有效 | 0/0 | 8（纯 walking-one） |

**教训**：`OV7670_RESET` 与 `OV7670_PWDN` 都会停 PCLK，任何"暂停采集"需求都应优先用
`FIFO_WR` 门控，而非动传感器复位/掉电引脚。
